...
calcularColision(palabras) {
  for (let i = palabras.length - 1; i >= 0; i--) {
    ...
    let manoX = this.x + 25 + this.spriteBrazo[0].width / 2;
    let manoY = 320 + this.spriteBrazo[0].height / 2;

    // Calcula las coordenadas del centro de la palabra
    let palabraX = palabra.x + palabra.t;
    let palabraY = palabra.y + palabra.t / 2;
